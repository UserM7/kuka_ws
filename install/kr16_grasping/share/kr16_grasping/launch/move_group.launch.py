...
def generate_launch_description():
    ...
    ompl_planning_yaml = load_yaml(
        "kuka_kr16_moveit_config", "config/ompl_planning.yaml"
    )
    ompl_planning_pipeline_config["move_group"].update(ompl_planning_yaml)

    # Trajectory Execution Functionality
    moveit_simple_controllers_yaml = load_yaml(
        "kuka_kr16_moveit_config", "config/kr16_controllers.yaml"
    )
    moveit_controllers = {
    ...
    # RViz
    rviz_config_file = (
        get_package_share_directory("kr16_grasping") + "/launch/move_group.rviz"
    )
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
